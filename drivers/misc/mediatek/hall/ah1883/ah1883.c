...
static int gpio_hall=-1;


static void ah1883_irq_work(struct work_struct *work)
{

	 ah1883data.hall_status = gpio_get_value(gpio_hall);
	 printk("==wuxiwen== hall   ah1883data.hall_status= %d\n",ah1883data.hall_status);
...
static irqreturn_t ah1883_irq(int irq, void *desc)
{
	...
	printk("==wuxiwen== hall   ah1883data.enable= %d\n",ah1883data.enable);
...
static struct device_attribute ah1883_attrs[] = {
	__ATTR(hall_status, S_IWUSR | S_IRUGO, show_hall_status,NULL),
	__ATTR(enable, S_IWUSR | S_IRUGO, show_enable,store_enable),
};
...
{
	int get_hall_gpio_num=-1;
	struct device_node *node = NULL;
	enum of_gpio_flags flags;

	printk("==wuxiwen==%s Device Tree get hall info!\n",__func__);

	node = of_find_compatible_node(NULL, NULL, "mediatek,mhall");
	if (node)
	{
		get_hall_gpio_num = of_get_named_gpio_flags(node, "hall_gpio_num", 0, &flags);
		printk("==wuxiwen==%s get_hall_gpio_num=%d\n",__func__,get_hall_gpio_num);
		if (get_hall_gpio_num >= 0) {
			if (gpio_is_valid(get_hall_gpio_num)) {
				if (!gpio_request(get_hall_gpio_num, "hall_gpio_num")) {
					printk("Device Tree: get hall_gpio_num ok!\n");
				}
			}
		}
	}
...
{
	int ret = -1;
	gpio_hall = get_hall_dts_func();
	if (-1==gpio_hall)
	{
		gpio_free(gpio_hall);
		return -1;
	}
	printk("==wuxiwen== hall fucn=%s,gpio_hall=%d\n",__func__,gpio_hall);
