...
static int mt_usb_dts_probe(struct platform_device *pdev)
{
	...
	if (node == NULL) {
		printk("==wuxiwen== get node failed\n");
	}else
	{
		usb_switch_pin = of_get_named_gpio(node, "usb_switch", 0);
		printk("==wuxiwen==%s usb_switch_pin=%d\n",__func__,usb_switch_pin);
		if (usb_switch_pin >= 0) {
		...
		{
			printk("[droi] usb_switch_pin faild!\n");
		}
	}
#endif
#endif
